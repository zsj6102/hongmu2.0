...
public class BrandApplyService extends Service {
    @Nullable
    @Override
    ...
    private void submit(Intent intent) {
        BrandApplyInfo info = (BrandApplyInfo) intent.getSerializableExtra("data");
        ...
        params.put("license_img\";filename=\"3.png", RequestBody.create(MediaType.parse("image/png"), info.getLicense_img()));
        params.put("license",OkhttpUtils.toRequestBody(info.getLicense()));
        params.put("physical_store_name",OkhttpUtils.toRequestBody(info.getPhysical_store_name()));
        ...
        RetrofitManager.getInstance(1, App.getInstance(), UrlConfig.PHILHARMONIC_HOST)
            ...
            public void onError(Throwable e) {
                ToastTools.showShort(BrandApplyService.this, "系统出错");
            }
            ...
            public void onNext(ApplyReturn applyReturn) {
                ...
                if (applyReturn.getCode() == 0) {
                    ToastTools.showShort(BrandApplyService.this, "提交成功");
                    RefreshMsg msg = new RefreshMsg();
                    msg.setType(23);//品牌二期
                    RxBus.get().post("refreshmsg", msg);
                }else if(applyReturn.getCode() == 1){
                    ToastTools.showShort(BrandApplyService.this, "系统异常");
                }else if(applyReturn.getCode() == 2){
                    ToastTools.showShort(BrandApplyService.this, "提交失败");
                }
            }
        });
    }
